...
class FindIdVC:BaseViewController,ViewControllerFromStoryboard{
    static func viewController() -> FindIdVC {
      let viewController = FindIdVC.viewController(storyBoardName: "Login")
    ...
    func initrx(){
        ...
        nextButton.rx.tapGesture().when(.recognized)
            ...
            guard let self = self else { return }
                ...
                let vc = FindIdOkVC.viewController()
